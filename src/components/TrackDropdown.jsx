...
export default function TracksDropdown() {
  ...
  const handleAddTrack = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`⚠️ ${data.message || "Failed to add track."}`);
      }
    } catch (err) {
  ...
  const handleUpdateTrack = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`⚠️ ${data.message || "Failed to update track."}`);
      }
    } catch (err) {
  ...
  const handleDeleteTrack = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        alert("🗑️ Track deleted successfully!");
        fetchTracks();
      } else {
        alert(`⚠️ ${data.message || "Failed to delete track."}`);
      }
    } catch (err) {
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <select
            ...
            onChange={(e) =>
              isEditing
                ? setEditTrack({ ...editTrack, department_id: e.target.value })
                : setNewTrack({ ...newTrack, department_id: e.target.value })
            }
            className="border rounded p-2 w-full"
            required
          >
